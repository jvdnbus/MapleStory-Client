...
namespace ms {
            ...
            for (uint8_t frame = 0; nl::node framenode = stancenode[frame]; ++frame) {
                for (nl::node layernode : framenode) {
                    ...
                    if (layer_iter == layers_by_name.end()) {
                        LOG(LOG_DEBUG,
                            "Unknown Hair::Layer name: [" << layername << "]\tLocation: [" << hairnode.name() << "][" <<
                            stancename << "][" << frame << "]");
                        continue;
                    }
                    ...
                    LOG(LOG_DEBUG,
                        "Invalid Hair::Layer texture\tName: [" << layername << "]\tLocation: [" << hairnode.name() <<
                        "][" << stancename << "][" << frame << "]");
                }
            }
        }
    ...
    {
        {"hair", DEFAULT},
        {"hairBelowBody", BELOWBODY},
        {"hairOverHead", OVERHEAD},
        {"hairShade", SHADE},
        {"backHair", BACK},
        {"backHairBelowCap", BELOWCAP},
        {"backHairBelowCapNarrow", BELOWCAPNARROW},
        {"backHairBelowCapWide", BELOWCAPWIDE}
    };
}
