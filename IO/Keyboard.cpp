...
namespace ms {
    constexpr int32_t Keytable[90] =
    {
        ...
        GLFW_KEY_0, GLFW_KEY_MINUS, GLFW_KEY_EQUAL,
        0, 0, // 15
        GLFW_KEY_Q, GLFW_KEY_W, GLFW_KEY_E, GLFW_KEY_R, GLFW_KEY_T, GLFW_KEY_Y, GLFW_KEY_U, GLFW_KEY_I, GLFW_KEY_O,
        GLFW_KEY_P, GLFW_KEY_LEFT_BRACKET, GLFW_KEY_RIGHT_BRACKET,
        0, // 28
        GLFW_KEY_LEFT_CONTROL, GLFW_KEY_A, GLFW_KEY_S, GLFW_KEY_D, GLFW_KEY_F, GLFW_KEY_G, GLFW_KEY_H, GLFW_KEY_J,
        GLFW_KEY_K, GLFW_KEY_L, GLFW_KEY_SEMICOLON, GLFW_KEY_APOSTROPHE, GLFW_KEY_GRAVE_ACCENT, GLFW_KEY_LEFT_SHIFT,
        GLFW_KEY_BACKSLASH, GLFW_KEY_Z, GLFW_KEY_X, GLFW_KEY_C, GLFW_KEY_V, GLFW_KEY_B, GLFW_KEY_N, GLFW_KEY_M,
        ...
        GLFW_KEY_F9, GLFW_KEY_F10, GLFW_KEY_F11, GLFW_KEY_F12, GLFW_KEY_HOME,
        0, // 72
        GLFW_KEY_PAGE_UP,
        0, 0, 0, 0, 0, // 78
        GLFW_KEY_END,
        0, // 80
        GLFW_KEY_PAGE_DOWN, GLFW_KEY_INSERT, GLFW_KEY_DELETE, GLFW_KEY_ESCAPE, GLFW_KEY_RIGHT_CONTROL,
        GLFW_KEY_RIGHT_SHIFT, GLFW_KEY_RIGHT_ALT, GLFW_KEY_SCROLL_LOCK
    };
    ...
    void Keyboard::assign(uint8_t key, uint8_t tid, int32_t action) {
        if (KeyType::Id type = KeyType::typebyid(tid)) {
            auto mapping = Mapping(type, action);
    ...
    void Keyboard::remove(uint8_t key) {
        auto mapping = Mapping(KeyType::Id::NONE, 0);
    ...
    Keyboard::Mapping Keyboard::get_text_mapping(int32_t keycode, bool shift) const {
        ...
        if (keycode == 39 || (keycode >= 44 && keycode <= 57) || keycode == 59 || keycode == 61 || (keycode >= 91
            && keycode <= 93) || keycode == 96) {
            if (!shift)
