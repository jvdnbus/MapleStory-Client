...
namespace ms {
    UIGender::UIGender(std::function<void()> oh) : UIElement(Point<int16_t>(0, 15), Point<int16_t>(0, 0)),
                                                   okhandler(oh) {
        CUR_TIMESTEP = 0;
        ...
        buttons[NO] = std::make_unique<MapleButton>(Gender["BtNo"], Point<int16_t>(650, 349));
        buttons[YES] = std::make_unique<MapleButton>(Gender["BtYes"], Point<int16_t>(578, 349));
        buttons[SELECT] = std::make_unique<MapleComboBox>(MapleComboBox::Type::DEFAULT, options,
    ...
    void UIGender::draw(float inter) const {
        auto gender_pos = Point<int16_t>(355, 185);

        if (CUR_TIMESTEP == 0) {
            gender_sprites[0].draw(position + gender_pos);
        } else if (CUR_TIMESTEP ==
            Constants::TIMESTEP * 3) {
            gender_sprites[1].draw(position + gender_pos);
        } else if (CUR_TIMESTEP >=
            Constants::TIMESTEP * 6) {
    ...
    Button::State UIGender::button_pressed(uint16_t buttonid) {
        ...
        case YES: {
            ...
            uint16_t selected_value = buttons[SELECT]->get_selected();
            GenderPacket(selected_value).dispatch();

            return Button::State::NORMAL;
        }
        case SELECT: {
            buttons[SELECT]->toggle_pressed();
