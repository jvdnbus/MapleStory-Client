...
namespace ms {
    UIQuestLog::UIQuestLog(const QuestLog& ql) : UIDragElement<PosQUEST>(), questlog(ql) {
        ...
        buttons[TAB0] = std::make_unique<TwoSpriteButton>(tabdis["0"], taben["0"]);
        buttons[TAB1] = std::make_unique<TwoSpriteButton>(tabdis["1"], taben["1"]);
        buttons[TAB2] = std::make_unique<TwoSpriteButton>(tabdis["2"], taben["2"]);
        buttons[CLOSE] = std::make_unique<MapleButton>(close, Point<int16_t>(275, 6));
        buttons[SEARCH] = std::make_unique<MapleButton>(list["BtSearch"]);
        buttons[ALL_LEVEL] = std::make_unique<MapleButton>(list["BtAllLevel"]);
        buttons[MY_LOCATION] = std::make_unique<MapleButton>(list["BtMyLocation"]);
        ...
        search = Textfield(Text::Font::A11M, Text::Alignment::LEFT, Color::Name::BOULDER,
                           Rectangle<int16_t>(get_search_pos(), get_search_pos() + get_search_dim()), search_limit);
    ...
    void UIQuestLog::draw(float alpha) const {
        ...
        auto notice_position = Point<int16_t>(0, 26);
    ...
    void UIQuestLog::change_tab(uint16_t tabid) {
        ...
        if (oldtab != tab) {
            buttons[TAB0 + oldtab]->set_state(Button::State::NORMAL);
            buttons[MY_LOCATION]->set_active(tab == TAB0);
            buttons[ALL_LEVEL]->set_active(tab == TAB0);
            buttons[SEARCH]->set_active(tab != TAB2);
