...
namespace ms {
    UISoftKey::UISoftKey(OkCallback ok_callback, CancelCallback cancel_callback, std::string tooltip_text,
                         Point<int16_t> tooltip_pos) : UIElement(Point<int16_t>(216, 224), Point<int16_t>(0, 0)),
                                                       highCase(false), ok_callback(ok_callback),
                                                       cancel_callback(cancel_callback), tooltip_pos(tooltip_pos) {
        nl::node SoftKey = nl::nx::UI["Login.img"]["Common"]["SoftKey"];
        nl::node backgrnd = SoftKey["backgrnd"];

        sprites.emplace_back(backgrnd);

        auto button_adj = Point<int16_t>(1, 0);

        buttons[BtCancel] = std::make_unique<MapleButton>(SoftKey["BtCancel"], button_adj);
        buttons[BtDel] = std::make_unique<MapleButton>(SoftKey["BtDel"], button_adj);
        buttons[BtOK] = std::make_unique<MapleButton>(SoftKey["BtOK"], button_adj);
        buttons[BtShift] = std::make_unique<MapleButton>(SoftKey["BtShift"], button_adj);
...
#pragma region Row 1
        ...
        for (uint16_t i = 0; i < ROW_MAX; i++) {
            ...
            if (key == "Blank")
                buttons[BtNum0 + i] = std::make_unique<MapleButton>(SoftKey["BtblankNum"], keypos(i, 0));
            else
                buttons[BtNum0 + i] = std::make_unique<MapleButton>(SoftKey["BtNum"][key], keypos(i, 0));
        }
#pragma endregion
#pragma endregion
...
#pragma region Row 2
        ...
        for (uint16_t i = 0; i < ROW_MAX; i++, caseKeyIndex++) {
            ...
            if (key == "Blank") {
                ...
                BtCaseKeys[caseKeyIndex][true] = std::make_unique<
                    MapleButton>(SoftKey["BtHighCase"][key], keypos(i, 1));
                BtCaseKeys[caseKeyIndex][false] = std::make_unique<
                    MapleButton>(SoftKey["BtLowCase"][key], keypos(i, 1));
            }
        }
#pragma endregion
...
#pragma region Row 3
        ...
        for (uint16_t i = 0; i < ROW_MAX; i++, caseKeyIndex++) {
            ...
            if (key == "Blank") {
                ...
                BtCaseKeys[caseKeyIndex][true] = std::make_unique<
                    MapleButton>(SoftKey["BtHighCase"][key], keypos(i, 2));
                BtCaseKeys[caseKeyIndex][false] = std::make_unique<
                    MapleButton>(SoftKey["BtLowCase"][key], keypos(i, 2));
            }
        }
#pragma endregion
...
#pragma region Row 4
        ...
        for (uint16_t i = 0; i < ROW4_MAX; i++, caseKeyIndex++) {
            ...
            if (key == "Blank") {
                ...
                BtCaseKeys[caseKeyIndex][true] = std::make_unique<
                    MapleButton>(SoftKey["BtHighCase"][key], keypos(i, 3));
                BtCaseKeys[caseKeyIndex][false] = std::make_unique<
                    MapleButton>(SoftKey["BtLowCase"][key], keypos(i, 3));
            }
        }
#pragma endregion
#pragma endregion
        ...
        textfield = Textfield(Text::Font::A11M, Text::Alignment::LEFT, Color::Name::BLACK,
                              Rectangle<int16_t>(textfield_topleft, textfield_topleft + Point<int16_t>(188, 21)),
                              MAX_TEXT_LEN);
    ...
    UISoftKey::UISoftKey(OkCallback ok_callback, CancelCallback cancel_callback) : UISoftKey(
    ...
    Cursor::State UISoftKey::send_cursor(bool clicked, Point<int16_t> cursorpos) {
        ...
        for (uint16_t i = 0; i < CASE_KEYS; i++) {
            ...
            if (btn->is_active() && btn->bounds(position).contains(cursorpos)) {
                ...
                if (btn->get_state() == Button::State::MOUSEOVER) {
                    if (clicked) {
    ...
    Button::State UISoftKey::button_pressed(uint16_t buttonid) {
        ...
        if (buttonid >= BtNum0) {
            std::string key = row1keys[buttonid - BtNum0];
    ...
    Button::State UISoftKey::case_pressed(uint16_t buttonid) {
        ...
        // Row 3
        if (buttonid >= ROW_MAX && buttonid < ROW_MAX * 2) {
        ...
        // Row 4
        if (buttonid >= ROW_MAX * 2 && buttonid < ROW_MAX * 2 + ROW4_MAX) {
