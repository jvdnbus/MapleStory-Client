...
namespace ms {
    UICommonCreation::UICommonCreation(const std::string& c) :
        UIElement(Point<int16_t>(0, 0), Point<int16_t>(800, 600)), female(false), genderSelected(false),
        charLookSelected(false), charNameSelected(false), newCharNameDisabled(true), classType(c),
        ...
        if (className == "cygnusKnight")
            sprites_gender_select.emplace_back(boardBottom,
                                               genderOrigin + Point<int16_t>(0, genderHeight - boardBottomHeight) -
                                               Point<int16_t>(0, 238));
        else
            sprites_gender_select.emplace_back(boardBottom,
                                               genderOrigin + Point<int16_t>(0, genderHeight - boardBottomHeight));
        ...
        if (className == "cygnusKnight")
            ...
            sprites_lookboard.emplace_back(boardBottom,
                                           genderOrigin + Point<int16_t>(0, avatarHeight - boardBottomHeight));

        nl::node avatarSel = CustomizeChar["avatarSel"];

        auto avatarSelAdj = Point<int16_t>(0, 0);
        ...
        for (size_t i = 0; i < avatarSel.size() - 1; i++) {
            ...
            if (className == "cygnusKnight") {
                if (i == 2)
                    sprites_char_look.emplace_back(avatarSel[i]["normal"],
                                                   avatarOrigin + avatarSelAdj - Point<int16_t>(0, 16));
                else if (i > 2)
                    sprites_char_look.emplace_back(avatarSel[i]["normal"],
                                                   avatarOrigin + avatarSelAdj - Point<int16_t>(0, 18));
                else
                    sprites_char_look.emplace_back(avatarSel[i]["normal"], avatarOrigin + avatarSelAdj);
            } else {
                sprites_char_look.emplace_back(avatarSel[i]["normal"],
                                               avatarOrigin + avatarSelAdj + Point<int16_t>(0, 18 * (t + 1)));
            }
        }
        ...
        buttons[BtStart] = std::make_unique<MapleButton>(Common["BtStart"], Point<int16_t>(0, 1));
        buttons[BtPreview] = std::make_unique<MapleButton>(Common["BtPreview"]);
        buttons[BtGenderMale] = std::make_unique<MapleButton>(genderSelect["male"], getGenderSelectMalePos());
        buttons[BtGenderFemale] = std::make_unique<MapleButton>(
            genderSelect["female"], getGenderSelectFemalePos());

        auto charLookAdj = Point<int16_t>(0, 0);
        ...
        buttons[BtSkinLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
        buttons[BtSkinRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);

        charLookLeft.shift_y(18);
        charLookRight.shift_y(18);

        buttons[BtFaceLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
        buttons[BtFaceRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);

        charLookLeft.shift_y(18);
        charLookRight.shift_y(18);

        buttons[BtHairLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
        buttons[BtHairRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);
        ...
        buttons[BtTopLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
        buttons[BtTopRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);

        charLookLeft.shift_y(18);
        charLookRight.shift_y(18);

        if (bottomEnabled) {
            buttons[BtBottomLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
            buttons[BtBottomRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);
        ...
        if (capeEnabled) {
            buttons[BtCapeLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
            buttons[BtCapeRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);

            charLookLeft.shift_y(18);
            charLookRight.shift_y(18);
        }

        buttons[BtShoesLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
        buttons[BtShoesRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);

        charLookLeft.shift_y(18);
        charLookRight.shift_y(18);

        buttons[BtWeaponLeft] = std::make_unique<MapleButton>(CustomizeChar["BtLeft"], charLookLeft);
        buttons[BtWeaponRight] = std::make_unique<MapleButton>(CustomizeChar["BtRight"], charLookRight);
        ...
        buttons[Buttons::BtYes] = std::make_unique<MapleButton>(
            BtYes, genderOrigin + Point<int16_t>(BtYesPos["gender"]));
        ...
        for (size_t i = 0; i < hairSelectSize; i++)
            buttons[BtHairSelect0 + i] = std::make_unique<MapleButton>(hairSelect[i], getHairSelectPos(i));
        ...
        auto newCharNameDim = Point<int16_t>(0, 0);
        ...
        newCharName = Textfield(Text::Font::A12M, Text::Alignment::LEFT, getNewCharNameColor(),
                                Rectangle<int16_t>(newCharNamePos, newCharNamePos + newCharNameDim), 12);
                ...
                if (i == 1) {
                    for (size_t f = 0; f < vCanvasCount; f++)
                        sprites_gender_select[i].draw(
                            position + Point<int16_t>(0, genderTopHeight + (boardMidHeight * f)), inter);
                } else {
            ...
            if (!nameused) {
                ...
                if (job >= 0) {
                    ...
                    auto onok = [&](bool alternate) {
                        ...
                        UI::get().remove(LOGINNOTICE_CONFIRM);
                        UI::get().remove(LOGINNOTICE);
                        UI::get().remove(CLASSCREATION);
                        UI::get().remove(RACESELECT);
                ...
                auto onok = [&]() {
                    ...
                    buttons[BtYes]->set_state(Button::State::NORMAL);
                    buttons[BtNo]->set_state(Button::State::NORMAL);
                };
    ...
    Button::State UICommonCreation::button_pressed(uint16_t buttonid) {
        ...
        if (buttonid == BtYes) {
            if (!genderSelected) {
                ...
                buttons[BtGenderMale]->set_active(false);
                buttons[BtGenderFemale]->set_active(false);
            ...
            if (!charLookSelected) {
                ...
                buttons[BtYes]->set_position(genderOrigin + Point<int16_t>(BtYesPos["name"]));
                buttons[BtNo]->set_position(genderOrigin + Point<int16_t>(BtNoPos["name"]));
            ...
            if (!charNameSelected) {
                ...
                if (name.size() >= 4) {
                    ...
                    buttons[BtYes]->set_state(Button::State::DISABLED);
                    buttons[BtNo]->set_state(Button::State::DISABLED);
        ...
        if (buttonid == BtNo) {
            ...
            if (genderSelected && choosableGender) {
                ...
                buttons[BtGenderMale]->set_active(true);
                buttons[BtGenderFemale]->set_active(true);

                disable_char_look();

                buttons[BtYes]->set_position(genderOrigin + Point<int16_t>(BtYesPos["gender"]));
                buttons[BtNo]->set_position(genderOrigin + Point<int16_t>(BtNoPos["gender"]));

                return Button::State::NORMAL;
            }
            button_pressed(BtPreview);

            return Button::State::NORMAL;
        }
        if (buttonid == BtSkinLeft) {
        ...
        if (buttonid >= BtHairSelect0 && buttonid < LENGTH) {
            hairColorIndex = buttonid - BtHairSelect0;
            ...
            for (size_t type = 0; type < gender_node.size(); type++) {
                for (nl::node id_node : gender_node[type]) {
                    ...
                    default: {
                        ...
                        if (id_node_prefix == 107) {
                            shoes[f].push_back(id_node_value);
                            break;
                        }
                        if (id_node_prefix == 110) {
    ...
    void UICommonCreation::enable_char_look() {
        buttons[BtYes]->set_position(genderOrigin + Point<int16_t>(BtYesPos["avatar"]));
        buttons[BtNo]->set_position(genderOrigin + Point<int16_t>(BtNoPos["avatar"]));

        if (faces[female].size() > 1) {
            buttons[BtFaceLeft]->set_active(true);
            buttons[BtFaceRight]->set_active(true);
        }

        if (hairStyles[female].size() > 1) {
            buttons[BtHairLeft]->set_active(true);
            buttons[BtHairRight]->set_active(true);
        }

        if (skinColors.size() > 1) {
            buttons[BtSkinLeft]->set_active(true);
            buttons[BtSkinRight]->set_active(true);
        }

        if (tops[female].size() > 1) {
            buttons[BtTopLeft]->set_active(true);
            buttons[BtTopRight]->set_active(true);
        }

        if (bottomEnabled && bottoms[female].size() > 1) {
            buttons[BtBottomLeft]->set_active(true);
            buttons[BtBottomRight]->set_active(true);
        }

        if (capeEnabled && capes[female].size() > 1) {
            buttons[BtCapeLeft]->set_active(true);
            buttons[BtCapeRight]->set_active(true);
        }

        if (shoes[female].size() > 1) {
            buttons[BtShoesLeft]->set_active(true);
            buttons[BtShoesRight]->set_active(true);
        }

        if (weapons[female].size() > 1) {
            buttons[BtWeaponLeft]->set_active(true);
            buttons[BtWeaponRight]->set_active(true);
        }

        if (hairColorEnabled)
            for (size_t i = 0; i < hairSelectSize; i++)
                buttons[BtHairSelect0 + i]->set_active(true);
    }

    void UICommonCreation::disable_char_look() {
        buttons[BtFaceLeft]->set_active(false);
        buttons[BtFaceRight]->set_active(false);
        buttons[BtHairLeft]->set_active(false);
        buttons[BtHairRight]->set_active(false);
        buttons[BtSkinLeft]->set_active(false);
        buttons[BtSkinRight]->set_active(false);
        buttons[BtTopLeft]->set_active(false);
        buttons[BtTopRight]->set_active(false);

        if (bottomEnabled) {
            buttons[BtBottomLeft]->set_active(false);
            buttons[BtBottomRight]->set_active(false);
        }

        if (capeEnabled) {
            buttons[BtCapeLeft]->set_active(false);
            buttons[BtCapeRight]->set_active(false);
        }

        buttons[BtShoesLeft]->set_active(false);
        buttons[BtShoesRight]->set_active(false);
        buttons[BtWeaponLeft]->set_active(false);
        buttons[BtWeaponRight]->set_active(false);

        for (size_t i = 0; i < hairSelectSize; i++)
            buttons[BtHairSelect0 + i]->set_active(false);
    }
    ...
    Point<int16_t> UICommonCreation::getGenderSelectMalePos() {
        auto genderSelectAdj = Point<int16_t>(-33, -13);
