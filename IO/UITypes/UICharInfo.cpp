...
namespace ms {
    UICharInfo::UICharInfo(int32_t cid) : UIDragElement<PosCHARINFO>(), is_loading(true), timestep(Constants::TIMESTEP),
                                          personality_enabled(false), collect_enabled(false), damage_enabled(false),
                                          item_enabled(false) {
        ...
        auto close_dimensions = Point<int16_t>(backgrnd_dim.x() - 21, 6);

        buttons[BtClose] = std::make_unique<MapleButton>(close, close_dimensions);
        buttons[BtCollect] = std::make_unique<MapleButton>(character["BtCollect"]);
        buttons[BtDamage] = std::make_unique<MapleButton>(character["BtDamage"]);
        buttons[BtFamily] = std::make_unique<MapleButton>(character["BtFamily"]);
        buttons[BtItem] = std::make_unique<MapleButton>(character["BtItem"]);
        buttons[BtParty] = std::make_unique<MapleButton>(character["BtParty"]);
        buttons[BtPersonality] = std::make_unique<MapleButton>(character["BtPersonality"]);
        buttons[BtPet] = std::make_unique<MapleButton>(character["BtPet"]);
        buttons[BtPopDown] = std::make_unique<MapleButton>(character["BtPopDown"]);
        buttons[BtPopUp] = std::make_unique<MapleButton>(character["BtPopUp"]);
        buttons[BtRide] = std::make_unique<MapleButton>(character["BtRide"]);
        buttons[BtTrad] = std::make_unique<MapleButton>(character["BtTrad"]);
        ...
        buttons[BtPet]->set_state(Button::State::DISABLED);
        buttons[BtRide]->set_state(Button::State::DISABLED);
        ...
        buttons[BtFriend] = std::make_unique<MapleButton>(farm["btFriend"], farm_adj);
        buttons[BtVisit] = std::make_unique<MapleButton>(farm["btVisit"], farm_adj);
...
#pragma region BottomWindow
        ...
        buttons[BtArrayGet] = std::make_unique<MapleButton>(collect["BtArrayGet"], bottom_window_adj);
        buttons[BtArrayName] = std::make_unique<MapleButton>(collect["BtArrayName"], bottom_window_adj);
        ...
        buttons[BtFAQ] = std::make_unique<MapleButton>(damage["BtFAQ"], bottom_window_adj);
        buttons[BtRegist] = std::make_unique<MapleButton>(damage["BtRegist"], bottom_window_adj);
...
#pragma endregion
    ...
    void UICharInfo::draw(float inter) const {
        ...
        if (collect_enabled) {
            ...
            for (size_t i = BtArrayGet; i < BtFAQ; i++)
                if (const auto button = buttons.at(i).get())
                    button->draw(position);

            auto text_pos = Point<int16_t>(121, 8);
        ...
        if (damage_enabled) {
            ...
            for (size_t i = BtFAQ; i < buttons.size(); i++)
                if (const auto button = buttons.at(i).get())
                    button->draw(position);
        }
    ...
    bool UICharInfo::is_in_range(Point<int16_t> cursorpos) const {
        ...
        auto farm_bounds = Rectangle<int16_t>(position, position + farm_dim);
        farm_bounds.shift(farm_adj);

        auto bottom_bounds = Rectangle<int16_t>(Point<int16_t>(0, 0), Point<int16_t>(0, 0));
        auto right_bounds = Rectangle<int16_t>(Point<int16_t>(0, 0), Point<int16_t>(0, 0));
    ...
    void UICharInfo::update_stats(int32_t character_id, int16_t job_id, uint16_t lv, int16_t f, std::string g,
        ...
        if (character_id == player_id) {
            ...
            buttons[BtPopDown]->set_state(Button::State::DISABLED);
            buttons[BtPopUp]->set_state(Button::State::DISABLED);
            buttons[BtFriend]->set_state(Button::State::DISABLED);
        }

        auto character_job = Job(job_id);
