...
namespace ms {
    UIQuit::UIQuit(const CharStats& st) : stats(st), screen_adj(212, 104) {
        ...
        buttons[NO] = std::make_unique<MapleButton>(askReward["btNo"], Point<int16_t>(0, 47));
        buttons[YES] = std::make_unique<MapleButton>(askReward["btYes"], Point<int16_t>(0, 47));
    ...
    void UIQuit::draw(float inter) const {
        ...
        auto time_adj = Point<int16_t>(188, -2);

        time_minutes.draw(time_minutes_text, time_number_width,
                          position + time_minutes_pos - time_adj + Point<int16_t>(50, 0));
