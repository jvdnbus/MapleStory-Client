...
namespace ms {
    UIChannel::UIChannel() : UIDragElement<PosCHANNEL>() {
        ...
        buttons[CANCEL] = std::make_unique<MapleButton>(Channel["BtCancel"]);
        buttons[CHANGE] = std::make_unique<MapleButton>(Channel["BtChange"], Point<int16_t>(-20, 0));
        ...
        for (size_t i = 0; i < channel_count; i++) {
            ...
            buttons[CH + i] = std::make_unique<AreaButton>(Point<int16_t>(11 + 70 * x, 55 + 20 * y),
    ...
    Cursor::State UIChannel::send_cursor(bool clicked, Point<int16_t> cursorpos) {
        ...
        for (size_t i = 0; i < channel_count + CH; i++) {
    ...
    Button::State UIChannel::button_pressed(uint16_t buttonid) {
        if (buttonid < CH) {
            ...
            if (buttonid - CH == current_channel)
                return Button::State::NORMAL;

            selected_channel = buttonid - CH;
            update_selected_channel_position();
        }
