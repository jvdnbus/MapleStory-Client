...
namespace ms {
    UIEquipInventory::UIEquipInventory(const Inventory& invent) : UIDragElement<PosEQINV>(), inventory(invent),
                                                                  tab(BT_TAB1), hasPendantSlot(false),
                                                                  hasPocketSlot(false) {
        // Column 1
        ...
        tab_source[BT_TAB0] = "Equip";
        tab_source[BT_TAB1] = "Cash";
        tab_source[BT_TAB2] = "Pet";
        tab_source[BT_TAB3] = "Android";

        nl::node close = nl::nx::UI["Basic.img"]["BtClose3"];
        nl::node Equip = nl::nx::UI["UIWindow4.img"]["Equip"];

        background[BT_TAB0] = Equip[tab_source[BT_TAB0]]["backgrnd"];
        background[BT_TAB1] = Equip[tab_source[BT_TAB1]]["backgrnd"];
        background[BT_TAB2] = Equip[tab_source[BT_TAB2]]["backgrnd"];
        background[BT_TAB3] = Equip[tab_source[BT_TAB3]]["backgrnd"];

        for (uint16_t i = BT_TAB0; i < BT_TABE; i++)
        ...
        tabbar = Equip["tabbar"];
        disabled = Equip[tab_source[BT_TAB0]]["disabled"];
        disabled2 = Equip[tab_source[BT_TAB0]]["disabled2"];

        buttons[BT_CLOSE] = std::make_unique<MapleButton>(close, Point<int16_t>(bg_dimensions.x() - 19, 5));
        buttons[BT_SLOT] = std::make_unique<MapleButton>(Equip[tab_source[BT_TAB0]]["BtSlot"]);
        buttons[BT_EFFECT] = std::make_unique<MapleButton>(EquipGL["Equip"]["btEffect"]);
        buttons[BT_SALON] = std::make_unique<MapleButton>(EquipGL["Equip"]["btSalon"]);
        buttons[BT_CONSUMESETTING] = std::make_unique<MapleButton>(
            Equip[tab_source[BT_TAB2]]["BtConsumeSetting"]);
        buttons[BT_EXCEPTION] = std::make_unique<MapleButton>(
            Equip[tab_source[BT_TAB2]]["BtException"]);
        buttons[BT_SHOP] = std::make_unique<MapleButton>(Equip[tab_source[BT_TAB3]]["BtShop"]);

        buttons[BT_CONSUMESETTING]->set_state(Button::State::DISABLED);
        buttons[BT_EXCEPTION]->set_state(Button::State::DISABLED);
        buttons[BT_SHOP]->set_state(Button::State::DISABLED);

        nl::node Tab = Equip["Tab"];

        for (uint16_t i = BT_TAB0; i < BT_TABE; i++)
            buttons[BT_TAB0 + i] = std::make_unique<TwoSpriteButton>(
                Tab["disabled"][i], Tab["enabled"][i], Point<int16_t>(0, 3));
    ...
    bool UIEquipInventory::is_in_range(Point<int16_t> cursorpos) const {
        ...
        auto totem_bounds = Rectangle<int16_t>(position, position + totem_dimensions);
    ...
    EquipSlot::Id UIEquipInventory::slot_by_position(Point<int16_t> cursorpos) const {
        ...
        for (auto iter : iconpositions) {
            auto iconrect = Rectangle<int16_t>(
    ...
    void UIEquipInventory::change_tab(uint16_t tabid) {
        ...
        if (oldtab != tab) {
            ...
            if (tab == BT_TAB0)
                buttons[BT_SLOT]->set_active(true);
            else
                buttons[BT_SLOT]->set_active(false);

            if (tab == BT_TAB2) {
                buttons[BT_CONSUMESETTING]->set_active(true);
                buttons[BT_EXCEPTION]->set_active(true);
            } else {
                buttons[BT_CONSUMESETTING]->set_active(false);
                buttons[BT_EXCEPTION]->set_active(false);
            }

            if (tab == BT_TAB3)
                buttons[BT_SHOP]->set_active(true);
            else
                buttons[BT_SHOP]->set_active(false);
        }
    }
