...
namespace ms {
    UIWorldMap::UIWorldMap() : UIDragElement<PosMAP>(), search_text_dim(Point<int16_t>(82, 14)) {
        ...
        auto close_dimensions = Point<int16_t>(bg_dimensions_x - 22, 4);

        buttons[BT_CLOSE] = std::make_unique<MapleButton>(close, close_dimensions);
        ...
        buttons[BT_SEARCH_CLOSE] = std::make_unique<MapleButton>(
            close, close_dimensions + Point<int16_t>(bg_search_dimensions_x, 0));
        buttons[BT_ALLSEARCH] = std::make_unique<MapleButton>(WorldMapSearch["BtAllsearch"],
                                                              background_dimensions);

        int16_t search_limit = 8;

        search_text = Textfield(Text::Font::A11M, Text::Alignment::LEFT, Color::Name::BLACK,
                                Rectangle<int16_t>(get_search_pos(), get_search_pos() + search_text_dim), search_limit);
        ...
        if (link_images.size() > 0) {
            for (auto& iter : buttons) {
                if (const auto button = iter.second.get()) {
                    if (iter.first >= BT_LINK0 && button->get_state() == Button::State::MOUSEOVER) {
    ...
    Cursor::State UIWorldMap::send_cursor(bool clicked, Point<int16_t> cursorpos) {
        ...
        for (auto path : map_spots) {
            ...
            auto abs_bounds = Rectangle<int16_t>(p, d);
            ...
            if (!clicked && abs_bounds.contains(cursorpos)) {
                ...
                UI::get().show_map(Tooltip::Parent::WORLDMAP, path.second.title, path.second.description,
                                   path.second.map_ids[0], path.second.bolded, false);
                break;
            }
        }
        ...
        for (nl::node list : WorldMap["MapList"]) {
            ...
            if (!desc && !title) {
                ...
                map_spots.emplace_back(std::make_pair<Point<int16_t>, MapSpot>(spot, {
                                                                                   map_info.description, path,
                                                                                   map_info.full_name, type, marker,
                                                                                   bolded, map_ids
                                                                               }));
            } else {
                map_spots.emplace_back(
                    std::make_pair<Point<int16_t>, MapSpot>(spot, {desc, path, title, type, marker, bolded, map_ids}));
            }
        }
    }
