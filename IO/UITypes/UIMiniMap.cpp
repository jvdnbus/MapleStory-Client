...
namespace ms {
    UIMiniMap::UIMiniMap(const CharStats& stats) : UIDragElement<PosMINIMAP>(Point<int16_t>(128, 20)), stats(stats) {
        ...
        buttons[BT_MIN] = std::make_unique<MapleButton>(MiniMap["BtMin"], Point<int16_t>(195, -6));
        buttons[BT_MAX] = std::make_unique<MapleButton>(MiniMap["BtMax"], Point<int16_t>(209, -6));
        buttons[BT_SMALL] = std::make_unique<MapleButton>(MiniMap["BtSmall"], Point<int16_t>(223, -6));
        buttons[BT_BIG] = std::make_unique<MapleButton>(MiniMap["BtBig"], Point<int16_t>(223, -6));
        buttons[BT_MAP] = std::make_unique<MapleButton>(MiniMap["BtMap"], Point<int16_t>(237, -6));
        ...
        marker = Setting<MiniMapDefaultHelpers>::get().load()
                     ? UIWindow2["MiniMapSimpleMode"]["DefaultHelper"]
                     : MapHelper["minimap"];
    ...
    void UIMiniMap::draw(float alpha) const {
        if (type == MIN) {
            ...
            if (has_map) {
                auto portal_marker = Animation(marker["portal"]);
    ...
    void UIMiniMap::update() {
        ...
        if (mid != mapid) {
            ...
            scale = std::pow(2, static_cast<int>(miniMap["mag"]));
    ...
    Cursor::State UIMiniMap::send_cursor(bool clicked, Point<int16_t> cursorpos) {
        ...
        if (listNpc_enabled) {
            Point<int16_t> relative_point = cursor_relative - Point<int16_t>(
                10 + (type == MAX ? max_dimensions : normal_dimensions).x(), 23);
            auto list_bounds = Rectangle<int16_t>(0, LISTNPC_ITEM_WIDTH, 0, LISTNPC_ITEM_HEIGHT * 8);
        ...
        for (auto npc = npcs->begin(); npc != npcs->end(); ++npc) {
            Point<int16_t> npc_pos = (npc->second->get_position() + center_offset) / scale + Point<int16_t>(
                map_draw_origin_x, map_draw_origin_y);
            auto marker_spot = Rectangle<int16_t>(npc_pos - Point<int16_t>(4, 8), npc_pos);
        ...
        if (!found) {
            for (auto& sprite : static_marker_info) {
                auto marker_spot = Rectangle<int16_t>(sprite.second, sprite.second + 8);
    ...
    Button::State UIMiniMap::button_pressed(uint16_t buttonid) {
        ...
        case BT_MIN: {
            ...
            return type == MIN ? Button::State::DISABLED : Button::State::NORMAL;
        }
        case BT_MAX: {
            type += 1;
            toggle_buttons();

            return type == MAX ? Button::State::DISABLED : Button::State::NORMAL;
        }
        case BT_SMALL:
        case BT_BIG: {
            big_map = !big_map;
    ...
    void UIMiniMap::toggle_buttons() {
        ...
        if (type == MIN) {
            ...
            if (big_map) {
                ...
                buttons[BT_BIG]->set_active(true);
                buttons[BT_SMALL]->set_active(false);
            }

            buttons[BT_MIN]->set_state(Button::State::NORMAL);

            bt_min_x = middle_right_x - (bt_min_width + buttons[BT_SMALL]->width() + 1 + bt_max_width +
                bt_map_width + (has_npcs ? buttons[BT_NPC]->width() : 0));
    ...
    void UIMiniMap::update_canvas() {
        ...
        int16_t window_width = std::max(178, std::max(static_cast<int>(mark_text_width), map_dimensions.x() + 20));
        ...
        int16_t dl_dr_y = std::max(map_dimensions.y(), static_cast<int16_t>(10));
        ...
        // Min sprites queue
        min_sprites.emplace_back(Min[Center],
        ...
        min_sprites.emplace_back(
            Min[Right], DrawArgument(WINDOW_UL_POS + Point<int16_t>(min_c_stretch + CENTER_START_X, 0)));
        ...
        normal_sprites.emplace_back(MiddleCenter,
        ...
        normal_sprites.emplace_back(Normal[MiddleRight],
                                    DrawArgument(Point<int16_t>(middle_right_x, ML_MR_Y),
                                                 Point<int16_t>(0, m_stretch)));
        normal_sprites.emplace_back(Normal[UpCenter],
        ...
        normal_sprites.emplace_back(Normal[DownCenter],
        ...
        max_sprites.emplace_back(Max[MiddleRight],
                                 DrawArgument(Point<int16_t>(middle_right_x, ML_MR_Y + MAX_ADJ),
                                              Point<int16_t>(0, m_stretch)));
        max_sprites.emplace_back(Max[UpCenter],
                                 DrawArgument(Point<int16_t>(CENTER_START_X, 0) + WINDOW_UL_POS,
                                              Point<int16_t>(c_stretch, 0)));
    ...
    void UIMiniMap::draw_movable_markers(Point<int16_t> init_pos, float alpha) const {
        ...
        for (auto npc = npcs->begin(); npc != npcs->end(); ++npc) {
            ...
            marker_sprite.draw(
                (npc_pos + center_offset) / scale - sprite_offset + Point<int16_t>(map_draw_origin_x, map_draw_origin_y)
                + init_pos, alpha);
        }
        ...
        for (auto chr = chars->begin(); chr != chars->end(); ++chr) {
            ...
            marker_sprite.draw(
                (chr_pos + center_offset) / scale - sprite_offset + Point<int16_t>(map_draw_origin_x, map_draw_origin_y)
                + init_pos, alpha);
        }
        ...
        player_marker.draw(
            (player_pos + center_offset) / scale - sprite_offset + Point<int16_t>(map_draw_origin_x, map_draw_origin_y)
            + init_pos, alpha);
    }
    ...
    void UIMiniMap::update_static_markers() {
        ...
        for (nl::node portal = portals.begin(); portal != portals.end(); ++portal) {
            ...
            if (portal_type == 2) {
                Point<int16_t> marker_pos = (Point<int16_t>(portal["x"], portal["y"]) + center_offset) / scale -
                    marker_offset + Point<int16_t>(map_draw_origin_x, map_draw_origin_y);
    ...
    void UIMiniMap::update_dimensions() {
        if (type == MIN) {
            ...
            Point<int16_t> base_dims = type == MAX ? max_dimensions : normal_dimensions;
            dimension = base_dims;
    ...
    void UIMiniMap::update_npclist() {
        ...
        for (auto npc = npcs->begin(); npc != npcs->end(); ++npc) {
            ...
            auto name_text = Text(Text::Font::A11M, Text::Alignment::LEFT, Color::Name::WHITE, name);
        ...
        const auto listNpc_pos = Point<int16_t>(
            type == MAX ? max_dimensions.x() : normal_dimensions.x(), 0);
        ...
        listNpc_sprites.emplace_back(listNpc["e"],
                                     DrawArgument(listNpc_pos + Point<int16_t>(CENTER_START_X + c_stretch, M_START),
                                                  Point<int16_t>(0, m_stretch)));
        listNpc_sprites.emplace_back(listNpc["n"],
        ...
        listNpc_sprites.emplace_back(listNpc["se"],
                                     DrawArgument(
                                         listNpc_pos + Point<int16_t>(CENTER_START_X + c_stretch,
                                                                      M_START + m_stretch)));
    ...
    void UIMiniMap::draw_npclist(Point<int16_t> minimap_dims, float alpha) const {
        auto npc_marker = Animation(marker["npc"]);
        ...
        for (int8_t i = 0; i + listNpc_offset < listNpc_list.size() && i < 8; i++) {
            if (selected - listNpc_offset == i) {
                auto highlight = ColorBox(LISTNPC_ITEM_WIDTH - (listNpc_slider.isenabled() ? 0 : 30),
        ...
        if (selected >= 0) {
            Point<int16_t> npc_pos =
                ...
                Point<int16_t>(map_draw_origin_x,
                               map_draw_origin_y - npc_marker.get_dimensions().y() + (type == MAX ? MAX_ADJ : 0));
