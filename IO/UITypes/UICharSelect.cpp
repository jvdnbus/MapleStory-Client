...
namespace ms {
    UICharSelect::UICharSelect(std::vector<CharEntry> characters, int8_t characters_count, int32_t slots,
                               int8_t require_pic) : UIElement(Point<int16_t>(0, 0), Point<int16_t>(800, 600)),
                                                     characters(characters), characters_count(characters_count),
                                                     slots(slots), require_pic(require_pic), tab_index(0),
                                                     tab_active(false), tab_move(false), charslot_y(0),
                                                     use_timestamp(false), burning_character(true),
                                                     show_pic_btns(false) {
        ...
        page_count = std::ceil(static_cast<double>(slots) / static_cast<double>(PAGESIZE));
        ...
        auto BtNewTexture = Texture(BtNew["normal"]["0"]);
        ...
        auto tab_adj = Point<int16_t>(107, 8);
        ...
        buttons[BtDelete] = std::make_unique<MapleButton>(CharSelect["BtDelete"]);
        buttons[BtPageL] = std::make_unique<MapleButton>(CharSelect["pageL"]);
        buttons[BtPageR] = std::make_unique<MapleButton>(CharSelect["pageR"]);
        buttons[BtChangePIC] = std::make_unique<MapleButton>(Common["BtChangePIC"]);
        buttons[BtResetPIC] = std::make_unique<MapleButton>(Login["WorldSelect"]["BtResetPIC"]);
        buttons[BtCharacter] = std::make_unique<MapleButton>(CharSelect["EditCharList"]["BtCharacter"]);
        ...
        auto screen_adj = Point<int16_t>(0, 3);
        ...
        for (size_t i = 0; i < PAGESIZE; i++)
            buttons[CHARACTER_SLOT0 + i] = std::make_unique<AreaButton>(
                get_character_slot_pos(i), avatarRB - avatarLT);

        if (require_pic == 0 || !show_pic_btns) {
            buttons[BtChangePIC]->set_active(false);
            buttons[BtResetPIC]->set_active(false);
        }
    ...
    void UICharSelect::draw(float inter) const {
        ...
        for (uint8_t i = 0; i < PAGESIZE; i++) {
            ...
            if (index < characters_count) {
                ...
                auto chararg = DrawArgument(charpos, flip_character);
    ...
    void UICharSelect::doubleclick(Point<int16_t> cursorpos) {
        uint16_t button_index = selected_character + CHARACTER_SLOT0;
        auto& btit = buttons[button_index];

        if (btit->is_active() && btit->bounds(position).contains(cursorpos) && btit->get_state() ==
            Button::State::NORMAL && button_index >= CHARACTER_SLOT0)
            button_pressed(BtSelect);
    }
            ...
            if (escape) {
                ...
                if (keycode == KeyAction::Id::TAB) {
                    ...
                    if (keycode == KeyAction::Id::UP) {
                        uint8_t next_index = (selected_index - COLUMNS < 0
                        ...
                        if (next_index >= index_total)
                            button_pressed(next_index - COLUMNS + CHARACTER_SLOT0);
                        else
                            button_pressed(next_index + CHARACTER_SLOT0);
                    } else if (keycode == KeyAction::Id::DOWN) {
                        uint8_t next_index = (selected_index + COLUMNS >= index_total
                                                  ? current_col
                                                  : selected_index + COLUMNS);
                        ...
                        if (selected_index != 0) {
                            ...
                            if (selected_index >= (selected_page + 1) * PAGESIZE - PAGESIZE)
                                button_pressed(selected_index + CHARACTER_SLOT0);
                            else
                                button_pressed(BtPageL);
                        }
    ...
    Button::State UICharSelect::button_pressed(uint16_t buttonid) {
        ...
        case BtPageR: {
            ...
            if (previous_page != selected_page) {
                ...
                button_pressed(CHARACTER_SLOT0);
            }
        ...
        default: {
            if (buttonid >= CHARACTER_SLOT0) {
                uint8_t previous_character = selected_character;
                selected_character = buttonid - CHARACTER_SLOT0 + selected_page * PAGESIZE;
    ...
    void UICharSelect::update_buttons() {
        for (uint8_t i = 0; i < PAGESIZE; i++) {
            ...
            if (index < characters_count)
                buttons[CHARACTER_SLOT0 + i]->set_state(Button::State::NORMAL);
            else
                buttons[CHARACTER_SLOT0 + i]->set_state(Button::State::DISABLED);
        }

        if (characters_count >= slots)
            buttons[BtNew]->set_state(Button::State::DISABLED);
        else
            buttons[BtNew]->set_state(Button::State::NORMAL);
        ...
        buttons[BtSelect]->set_active(character_found);
        buttons[BtDelete]->set_state(character_found ? Button::State::NORMAL : Button::State::DISABLED);
    }
    ...
    void UICharSelect::select_last_slot() {
        for (int8_t i = PAGESIZE - 1; i >= 0; i--) {
            ...
            if (index < characters_count) {
                button_pressed(i + CHARACTER_SLOT0);
