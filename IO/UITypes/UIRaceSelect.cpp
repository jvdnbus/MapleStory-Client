...
namespace ms {
    UIRaceSelect::UIRaceSelect() : UIElement(Point<int16_t>(0, 0), Point<int16_t>(800, 600)) {
        ...
        for (size_t i = 0; i < tempOrder.size(); i++)
            if (tempOrder[i].get_integer() != PINKBEAN)
        ...
        class_isdisabled[EXPLORERS] = false;
        class_isdisabled[CYGNUSKNIGHTS] = false;
        class_isdisabled[ARAN] = false;
        ...
        for (nl::node node : button) {
            ...
            if (name == "intervalX" || name == "pos" || name == std::to_string(PINKBEAN) || name ==
                std::to_string(YETI))
                continue;
        ...
        buttons[BtMake] = std::make_unique<MapleButton>(RaceSelect["make"]);
        buttons[BtLeftArrow] = std::make_unique<MapleButton>(RaceSelect["leftArrow"]);
        buttons[BtRightArrow] = std::make_unique<MapleButton>(RaceSelect["rightArrow"]);

        for (size_t i = 0; i <= CLASS0; i++)
            buttons[CLASS0 + i] = std::make_unique<AreaButton>(
                get_class_pos(i), class_normal[0][true].get_dimensions());

        index_shift = 0;
        selected_index = 0;
        selected_class = class_index[selected_index];

        buttons[BtLeftArrow]->set_state(Button::State::DISABLED);
    ...
    void UIRaceSelect::update() {
        ...
        if (selected_class == ZERO)
            buttons[BtMake]->set_position(position + posZero);
        else
            buttons[BtMake]->set_position(position + pos);
        ...
        for (auto& btit : buttons) {
            if (btit.second->is_active() && btit.second->bounds(position).contains(cursorpos)) {
                if (btit.second->get_state() == Button::State::NORMAL) {
                    ...
                    if (btit.first >= CLASS0)
                        mouseover[btit.first - CLASS0] = true;
                ...
                if (btit.first >= CLASS0)
                    mouseover[btit.first - CLASS0] = false;
    ...
    void UIRaceSelect::send_key(int32_t keycode, bool pressed, bool escape) {
        if (pressed) {
            if (escape) {
                show_charselect();
            } else if (keycode == KeyAction::Id::LEFT || keycode ==
                KeyAction::Id::DOWN) {
                if (buttons[BtLeftArrow]->get_state() == Button::State::NORMAL)
                    button_pressed(BtLeftArrow);
            } else if (keycode == KeyAction::Id::RIGHT || keycode == KeyAction::Id::UP) {
                if (buttons[BtRightArrow]->get_state() == Button::State::NORMAL)
                    button_pressed(BtRightArrow);
            } else if (keycode == KeyAction::Id::RETURN) {
                button_pressed(BtMake);
            }
        }
    }
    ...
    Button::State UIRaceSelect::button_pressed(uint16_t buttonid) {
        ...
        if (buttonid == BtLeftArrow) {
            uint16_t new_index = selected_index - 1;

            if (selected_index - index_shift == 0) {
                index_shift--;

                class_index[0] = order[new_index + 5 - CLASS0];
                class_index[1] = order[new_index + 6 - CLASS0];
                class_index[2] = order[new_index + 7 - CLASS0];
                class_index[3] = order[new_index + 8 - CLASS0];
                class_index[4] = order[new_index + 9 - CLASS0];
                class_index[5] = order[new_index + 10 - CLASS0];
            }
        ...
        if (buttonid == BtRightArrow) {
            ...
            if (selected == class_index[5]) {
                ...
                class_index[0] = order[new_index + 0 - CLASS0];
                class_index[1] = order[new_index + 1 - CLASS0];
                class_index[2] = order[new_index + 2 - CLASS0];
                class_index[3] = order[new_index + 3 - CLASS0];
                class_index[4] = order[new_index + 4 - CLASS0];
                class_index[5] = order[new_index + 5 - CLASS0];
            }
        ...
        if (buttonid >= CLASS0) {
            uint16_t index = buttonid - CLASS0 + index_shift;
    ...
    void UIRaceSelect::select_class(uint16_t index) {
        ...
        if (previous_index != selected_index) {
            ...
            buttons[previous + CLASS0]->set_state(Button::State::NORMAL);
            ...
            } else if (class_index[previous] == KINESIS) {
        ...
        if (selected_index > 0)
            buttons[BtLeftArrow]->set_state(Button::State::NORMAL);
        else
            buttons[BtLeftArrow]->set_state(Button::State::DISABLED);

        if (selected_index < order.size() - 1)
            buttons[BtRightArrow]->set_state(Button::State::NORMAL);
        else
            buttons[BtRightArrow]->set_state(Button::State::DISABLED);
    }
