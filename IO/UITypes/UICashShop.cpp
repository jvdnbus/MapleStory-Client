...
namespace ms {
    UICashShop::UICashShop() : preview_index(0), menu_index(1), promotion_index(0), mvp_grade(1), mvp_exp(0.07f),
                               list_offset(0) {
        ...
        for (size_t i = 0; i < 3; i++)
            buttons[BtPreview1 + i] = std::make_unique<TwoSpriteButton>(
                Base["Tab"]["Disable"][i], Base["Tab"]["Enable"][i], Point<int16_t>(957 + (i * 17), 46));

        buttons[BtPreview1]->set_state(Button::State::PRESSED);

        buttons[BtExit] = std::make_unique<MapleButton>(CSTab["BtExit"], Point<int16_t>(5, 728));
        buttons[BtChargeNX] = std::make_unique<
            MapleButton>(CSGLChargeNX["BtChargeNX"], Point<int16_t>(5, 554));
        buttons[BtChargeRefresh] = std::make_unique<MapleButton>(
            CSGLChargeNX["BtChargeRefresh"], Point<int16_t>(92, 554));
        ...
        buttons[BtChargeRefresh] = std::make_unique<MapleButton>(
            CSGLChargeNX["BtChargeRefresh"], Point<int16_t>(92, 554));
        buttons[BtWish] = std::make_unique<MapleButton>(CSStatus["BtWish"], Point<int16_t>(226, 6));
        buttons[BtMileage] = std::make_unique<MapleButton>(CSStatus["BtMileage"], Point<int16_t>(869, 4));
        buttons[BtHelp] = std::make_unique<MapleButton>(CSStatus["BtHelp"], Point<int16_t>(997, 4));
        buttons[BtCoupon] = std::make_unique<MapleButton>(CSStatus["BtCoupon"], Point<int16_t>(950, 4));
        ...
        buttons[BtNext] = std::make_unique<MapleButton>(CSPromotionBanner["BtNext"], promotion_pos);
        buttons[BtPrev] = std::make_unique<MapleButton>(CSPromotionBanner["BtPrev"], promotion_pos);

        for (size_t i = 0; i < 7; i++)
            mvp_sprites[i] = CSMVPBanner["grade"][i];

        mvp_pos = Point<int16_t>(63, 681);
        buttons[BtDetailPackage] = std::make_unique<MapleButton>(CSMVPBanner["BtDetailPackage"], mvp_pos);
        buttons[BtNonGrade] = std::make_unique<MapleButton>(CSMVPBanner["BtNonGrade"], mvp_pos);

        buttons[BtDetailPackage]->set_active(mvp_grade);
        buttons[BtNonGrade]->set_active(!mvp_grade);
        ...
        auto search_pos = Point<int16_t>(0, 36);
        ...
        buttons[BtBuyAvatar] = std::make_unique<MapleButton>(CSChar["BtBuyAvatar"], Point<int16_t>(642, 305));
        buttons[BtDefaultAvatar] = std::make_unique<MapleButton>(
            CSChar["BtDefaultAvatar"], Point<int16_t>(716, 305));
        buttons[BtInventory] = std::make_unique<MapleButton>(CSChar["BtInventory"], Point<int16_t>(938, 305));
        buttons[BtSaveAvatar] = std::make_unique<
            MapleButton>(CSChar["BtSaveAvatar"], Point<int16_t>(864, 305));
        buttons[BtTakeoffAvatar] = std::make_unique<MapleButton>(
            CSChar["BtTakeoffAvatar"], Point<int16_t>(790, 305));
        ...
        for (size_t i = 0; i < MAX_ITEMS; i++) {
            ...
            buttons[BtBuy + i] = std::make_unique<MapleButton>(
                CSList["BtBuy"], Point<int16_t>(146, 523) + Point<int16_t>(124 * div.rem, 205 * div.quot));
        ...
        auto slider_pos = Point<int16_t>(1007, 372);
    ...
    void UICashShop::draw(float inter) const {
        ...
        if (items.size() > 0)
            ...
            item_none.draw(
                position + Point<int16_t>(137, 372) + Point<int16_t>(BestNew_dim.x() / 2,
                                                                     list_slider.getvertical().length() / 2) - item_none
                .get_dimensions() / 2, inter);
        ...
        for (size_t i = 0; i < MAX_ITEMS; i++) {
            ...
            if (index < items.size()) {
                ...
                item.draw(DrawArgument(
                    position + Point<int16_t>(164, 473) + Point<int16_t>(124 * div.rem, 205 * div.quot), 2.0f, 2.0f));
                ...
                if (item_discount[i].get_text() == "") {
                    ...
                    item_percent[i].draw(
                        position + Point<int16_t>(198 + (item_discount[i].width() / 2), 495) + Point<int16_t>(
                            124 * div.rem, 205 * div.quot));
                }
            }
        }
    ...
    Button::State UICashShop::button_pressed(uint16_t buttonid) {
        ...
        if (buttonid >= BtBuy) {
            int16_t index = buttonid - BtBuy + list_offset;
    ...
    void UICashShop::exit_cashshop() {
        ...
        PlayerLoginPacket(player.get_object_id()).dispatch();
    ...
    void UICashShop::update_items() {
        for (size_t i = 0; i < MAX_ITEMS; i++) {
            ...
            buttons[BtBuy + i]->set_active(found_item);
            ...
            if (found_item) {
                ...
                if (item.discount_price > 0 && price > 0) {
                    ...
                    float_t percent = static_cast<float>(discount) / price;
