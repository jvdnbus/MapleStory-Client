...
namespace ms {
    UIItemInventory::UIItemInventory(const Inventory& invent) : UIDragElement<PosINV>(), inventory(invent),
                                                                tab(InventoryType::Id::EQUIP), ignore_tooltip(false),
                                                                sort_enabled(true) {
        ...
        auto tab_pos_adj = Point<int16_t>(9, 26);

        nl::node close = nl::nx::UI["Basic.img"]["BtClose3"];
        buttons[BT_CLOSE] = std::make_unique<MapleButton>(close);

        buttons[BT_TAB_EQUIP] = std::make_unique<TwoSpriteButton>(
            tabdis["0"], taben["0"], tab_pos0 - tab_pos_adj, Point<int16_t>(0, 0));
        buttons[BT_TAB_USE] = std::make_unique<TwoSpriteButton>(
            tabdis["1"], taben["1"], tab_pos1 - tab_pos_adj, Point<int16_t>(0, 0));
        buttons[BT_TAB_ETC] = std::make_unique<TwoSpriteButton>(
            tabdis["2"], taben["2"], tab_pos2 - tab_pos_adj, Point<int16_t>(0, 0));
        buttons[BT_TAB_SETUP] = std::make_unique<TwoSpriteButton>(
            tabdis["3"], taben["3"], tab_pos3 - tab_pos_adj, Point<int16_t>(0, 0));
        buttons[BT_TAB_CASH] = std::make_unique<TwoSpriteButton>(
            tabdis["4"], taben["4"], tab_pos4 - tab_pos_adj, Point<int16_t>(0, 0));
        buttons[BT_TAB_DEC] = std::make_unique<TwoSpriteButton>(
            tabdis["5"], taben["5"], tab_pos5 - tab_pos_adj, Point<int16_t>(0, 0));

        buttons[BT_COIN] = std::make_unique<MapleButton>(AutoBuild["button:Coin"]);
        buttons[BT_POINT] = std::make_unique<MapleButton>(AutoBuild["button:Point"]);
        buttons[BT_GATHER] = std::make_unique<MapleButton>(AutoBuild["button:Gather"]);
        buttons[BT_SORT] = std::make_unique<MapleButton>(AutoBuild["button:Sort"]);
        buttons[BT_FULL] = std::make_unique<MapleButton>(AutoBuild["button:Full"]);
        buttons[BT_UPGRADE] = std::make_unique<MapleButton>(AutoBuild["button:Upgrade"]);
        buttons[BT_APPRAISE] = std::make_unique<MapleButton>(AutoBuild["button:Appraise"]);
        buttons[BT_EXTRACT] = std::make_unique<MapleButton>(AutoBuild["button:Extract"]);
        buttons[BT_DISASSEMBLE] = std::make_unique<MapleButton>(AutoBuild["button:Disassemble"]);
        buttons[BT_TOAD] = std::make_unique<MapleButton>(AutoBuild["anibutton:Toad"]);

        buttons[BT_SMALL] = std::make_unique<MapleButton>(FullAutoBuild["button:Small"]);
        buttons[BT_COIN_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Coin"]);
        buttons[BT_POINT_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Point"]);
        buttons[BT_GATHER_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Gather"]);
        buttons[BT_SORT_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Sort"]);
        buttons[BT_UPGRADE_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Upgrade"]);
        buttons[BT_APPRAISE_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Appraise"]);
        buttons[BT_EXTRACT_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Extract"]);
        buttons[BT_DISASSEMBLE_SM] = std::make_unique<MapleButton>(FullAutoBuild["button:Disassemble"]);
        buttons[BT_TOAD_SM] = std::make_unique<MapleButton>(FullAutoBuild["anibutton:Toad"]);
        buttons[BT_CASHSHOP] = std::make_unique<MapleButton>(FullAutoBuild["button:Cashshop"]);

        buttons[BT_EXTRACT]->set_state(Button::State::DISABLED);
        buttons[BT_EXTRACT_SM]->set_state(Button::State::DISABLED);
        buttons[BT_DISASSEMBLE]->set_state(Button::State::DISABLED);
        buttons[BT_DISASSEMBLE_SM]->set_state(Button::State::DISABLED);
                ...
                case InventoryType::Id::EQUIP: {
                    if (can_wear_equip(slot)) {
                        ...
                        if (equipslot == EquipSlot::Id::NONE) {
                            LOG(LOG_DEBUG,
                                "Could not find appropriate EquipSlot::Id for item [" << item_id <<
                                "]. Equip would be dropped.");
                            break;
                        }
    ...
    void UIItemInventory::set_sort(bool enabled) {
        ...
        if (full_enabled) {
            if (sort_enabled) {
                ...
                buttons[BT_SORT_SM]->set_active(true);
                buttons[BT_GATHER]->set_active(false);
                buttons[BT_GATHER_SM]->set_active(false);
            } else {
                buttons[BT_SORT]->set_active(false);
                buttons[BT_SORT_SM]->set_active(false);
                buttons[BT_GATHER]->set_active(false);
                buttons[BT_GATHER_SM]->set_active(true);
            }
        } else {
            if (sort_enabled) {
    ...
    int16_t UIItemInventory::slot_by_position(Point<int16_t> cursorpos) const {
        ...
        int16_t xmin = slot_x;
        int16_t xmax = (icon_width + slot_space_x) * (full_enabled ? slot_col * 4 : slot_col) - (
            full_enabled ? slot_space_x : 0);

        int16_t cur_y = cursorpos.y();
        int16_t slot_y = slot_pos.y();
        int16_t ymin = slot_y;
        int16_t ymax = (icon_height + slot_space_y) * (full_enabled ? slot_row + 1 : slot_row - 1) - (
            full_enabled ? slot_space_y : 0);
    ...
    void UIItemInventory::set_full(bool enabled) {
        ...
        if (full_enabled) {
            ...
            buttons[BT_FULL]->set_active(false);
            buttons[BT_SMALL]->set_active(true);
        } else {
            dimension = bg_dimensions;

            buttons[BT_FULL]->set_active(true);
            buttons[BT_SMALL]->set_active(false);
        }

        dragarea = Point<int16_t>(dimension.x(), 20);

        buttons[BT_CLOSE]->set_position(Point<int16_t>(dimension.x() - 20, 6));

        buttons[BT_COIN]->set_active(!enabled);
        buttons[BT_POINT]->set_active(!enabled);
        buttons[BT_UPGRADE]->set_active(!enabled);
        buttons[BT_APPRAISE]->set_active(!enabled);
        buttons[BT_EXTRACT]->set_active(!enabled);
        buttons[BT_DISASSEMBLE]->set_active(!enabled);
        buttons[BT_TOAD]->set_active(!enabled);
        buttons[BT_CASHSHOP]->set_active(!enabled);

        buttons[BT_COIN_SM]->set_active(enabled);
        buttons[BT_POINT_SM]->set_active(enabled);
        buttons[BT_UPGRADE_SM]->set_active(enabled);
        buttons[BT_APPRAISE_SM]->set_active(enabled);
        buttons[BT_EXTRACT_SM]->set_active(enabled);
        buttons[BT_DISASSEMBLE_SM]->set_active(enabled);
        buttons[BT_TOAD_SM]->set_active(enabled);
        buttons[BT_CASHSHOP]->set_active(enabled);
    ...
    void UIItemInventory::ItemIcon::drop_on_stage() const {
        constexpr auto dropmessage = "How many will you drop?";
        constexpr auto untradablemessage =
            "This item can't be taken back once thrown away.\\nWill you still drop it?";
        constexpr auto cashmessage = "You can't drop this item.";
    ...
    void UIItemInventory::ItemIcon::drop_on_bindings(Point<int16_t> cursorposition, bool remove) const {
        if (sourcetab == InventoryType::Id::USE || sourcetab == InventoryType::Id::SETUP) {
            ...
            auto mapping = Keyboard::Mapping(KeyType::ITEM, item_id);
