...
namespace ms {
    ...
    void UISkillBook::SkillIcon::drop_on_bindings(Point<int16_t> cursorposition, bool remove) const {
        ...
        auto mapping = Keyboard::Mapping(KeyType::SKILL, skill_id);
    ...
    UISkillBook::UISkillBook(const CharStats& in_stats, const SkillBook& in_skillbook) : UIDragElement<PosSKILL>(),
                                                                                         stats(in_stats), skillbook(in_skillbook), tab(0), grabbing(false), macro_enabled(false), sp_enabled(false) {
        ...
        buttons[BT_HYPER] = std::make_unique<MapleButton>(main["BtHyper"]);
        buttons[BT_GUILDSKILL] = std::make_unique<MapleButton>(main["BtGuildSkill"]);
        buttons[BT_RIDE] = std::make_unique<MapleButton>(main["BtRide"]);
        buttons[BT_MACRO] = std::make_unique<MapleButton>(main["BtMacro"]);

        buttons[BT_HYPER]->set_state(Button::State::DISABLED);
        buttons[BT_GUILDSKILL]->set_state(Button::State::DISABLED);
        buttons[BT_RIDE]->set_state(Button::State::DISABLED);
        ...
        buttons[BT_CANCLE] = std::make_unique<MapleButton>(skillPoint["BtCancle"],
                                                           Point<int16_t>(bg_dimensions.x(), 0));
        buttons[BT_OKAY] = std::make_unique<MapleButton>(skillPoint["BtOkay"],
                                                         Point<int16_t>(bg_dimensions.x(), 0));
        buttons[BT_SPDOWN] = std::make_unique<MapleButton>(skillPoint["BtSpDown"],
                                                           Point<int16_t>(bg_dimensions.x(), 0));
        buttons[BT_SPMAX] = std::make_unique<MapleButton>(skillPoint["BtSpMax"],
                                                          Point<int16_t>(bg_dimensions.x(), 0));
        buttons[BT_SPUP] = std::make_unique<MapleButton>(skillPoint["BtSpUp"],
                                                         Point<int16_t>(bg_dimensions.x(), 0));

        buttons[BT_SPDOWN]->set_state(Button::State::DISABLED);
        ...
        buttons[BT_MACRO_OK] = std::make_unique<MapleButton>(macro["BtOK"],
                                                             Point<int16_t>(bg_dimensions.x(), 0));

        buttons[BT_MACRO_OK]->set_state(Button::State::DISABLED);

        nl::node close = nl::nx::UI["Basic.img"]["BtClose3"];

        buttons[BT_CLOSE] = std::make_unique<MapleButton>(close, Point<int16_t>(bg_dimensions.x() - 23, 6));
        ...
        for (uint16_t i = BT_SPUP0; i <= BT_SPUP11; ++i) {
            uint16_t x_adj = 0;
    ...
    Button::State UISkillBook::button_pressed(uint16_t id) {
        ...
        case BT_SPDOWN: {
            ...
            buttons[BT_SPUP]->set_state(Button::State::NORMAL);
            buttons[BT_SPMAX]->set_state(Button::State::NORMAL);
        ...
        case BT_SPMAX: {
            ...
            buttons[BT_SPUP]->set_state(Button::State::DISABLED);
            buttons[BT_SPDOWN]->set_state(Button::State::NORMAL);
        ...
        case BT_SPUP: {
            ...
            if (sp_after == sp_masterlevel) {
                buttons[BT_SPMAX]->set_state(Button::State::DISABLED);
    ...
    Cursor::State UISkillBook::send_cursor(bool clicked, Point<int16_t> cursorpos) {
        ...
        if (!grabbing) {
            for (size_t i = 0; i < skills.size(); i++) {
                ...
                constexpr auto bounds = Rectangle<int16_t>(0, 32, 0, 32);
    ...
    void UISkillBook::change_sp() {
        ...
        if (joblevel == Job::Level::BEGINNER) {
            ...
            for (size_t i = 0; i < skills.size(); i++) {
                ...
                if (skillid == SkillId::Id::THREE_SNAILS || skillid == SkillId::Id::HEAL || skillid ==
                    SkillId::Id::FEATHER)
    ...
    void UISkillBook::set_macro(bool enabled) {
        ...
        buttons[BT_MACRO_OK]->set_active(macro_enabled);
    ...
    void UISkillBook::set_skillpoint(bool enabled) {
        ...
        buttons[BT_CANCLE]->set_active(sp_enabled);
        buttons[BT_OKAY]->set_active(sp_enabled);
        buttons[BT_SPDOWN]->set_active(sp_enabled);
        buttons[BT_SPMAX]->set_active(sp_enabled);
        buttons[BT_SPUP]->set_active(sp_enabled);
