...
namespace ms {
    UINpcTalk::UINpcTalk() : offset(0), unitrows(0), rowmax(0), show_slider(false), draw_text(false),
        ...
        buttons[ALLLEVEL] = std::make_unique<MapleButton>(UtilDlgEx["BtAllLevel"]);
        buttons[CLOSE] = std::make_unique<MapleButton>(UtilDlgEx["BtClose"]);
        buttons[MYLEVEL] = std::make_unique<MapleButton>(UtilDlgEx["BtMyLevel"]);
        buttons[NEXT] = std::make_unique<MapleButton>(UtilDlgEx["BtNext"]);
        buttons[NO] = std::make_unique<MapleButton>(UtilDlgEx["BtNo"]);
        buttons[OK] = std::make_unique<MapleButton>(UtilDlgEx["BtOK"]);
        buttons[PREV] = std::make_unique<MapleButton>(UtilDlgEx["BtPrev"]);
        buttons[QAFTER] = std::make_unique<MapleButton>(UtilDlgEx["BtQAfter"]);
        buttons[QCNO] = std::make_unique<MapleButton>(UtilDlgEx["BtQCNo"]);
        buttons[QCYES] = std::make_unique<MapleButton>(UtilDlgEx["BtQCYes"]);
        buttons[QGIVEUP] = std::make_unique<MapleButton>(UtilDlgEx["BtQGiveup"]);
        buttons[QNO] = std::make_unique<MapleButton>(UtilDlgEx["BtQNo"]);
        buttons[QSTART] = std::make_unique<MapleButton>(UtilDlgEx["BtQStart"]);
        buttons[QYES] = std::make_unique<MapleButton>(UtilDlgEx["BtQYes"]);
        buttons[YES] = std::make_unique<MapleButton>(UtilDlgEx["BtYes"]);
    ...
    void UINpcTalk::change_text(int32_t npcid, int8_t msgtype, int16_t, int8_t speakerbyte, const std::string& tx) {
        ...
        buttons[CLOSE]->set_position(Point<int16_t>(9, y_cord));
        buttons[CLOSE]->set_active(true);

        switch (type) {
        case SENDOK: {
            buttons[OK]->set_position(Point<int16_t>(471, y_cord));
            buttons[OK]->set_active(true);
            break;
        }
        case SENDYESNO: {
            auto yes_position = Point<int16_t>(389, y_cord);

            buttons[YES]->set_position(yes_position);
            buttons[YES]->set_active(true);

            buttons[NO]->set_position(yes_position + Point<int16_t>(65, 0));
