...
namespace ms {
    UITermsOfService::UITermsOfService(std::function<void()> oh) : okhandler(oh), offset(0), unit_rows(1) {
        ...
        buttons[OK] = std::make_unique<MapleButton>(Login["BtOk"], Point<int16_t>(483, 425));
        buttons[CANCEL] = std::make_unique<MapleButton>(Login["BtCancel"], Point<int16_t>(557, 425));
    ...
    void UITermsOfService::update_accept(uint16_t offset) {
        if (offset == max_rows - unit_rows)
            ...
            buttons[OK]->set_state(Button::State::DISABLED);
