...
namespace ms {
    ...
    void UI::send_key(int32_t keycode, bool pressed) {
        if ((is_key_down[GLFW_KEY_LEFT_ALT] || is_key_down[GLFW_KEY_RIGHT_ALT]) && (is_key_down[GLFW_KEY_ENTER] ||
            is_key_down[GLFW_KEY_KP_ENTER])) {
        ...
        if (focusedtextfield) {
            ...
            auto statusbar = get().get_element<UIStatusBar>();
            auto channel = get().get_element<UIChannel>();
            auto worldmap = get().get_element<UIWorldMap>();
            auto optionmenu = get().get_element<UIOptionMenu>();
            auto shop = get().get_element<UIShop>();
            auto joypad = get().get_element<UIJoypad>();
            auto rank = get().get_element<UIRank>();
            auto quit = get().get_element<UIQuit>();
            auto npctalk = get().get_element<UINpcTalk>();
            ...
            if (npctalk && npctalk->is_active()) {
                ...
                if (escape || tab || enter || arrows) {
                    ...
                    types.emplace_back(UIElement::Type::CLASSCREATION);
                    // No tab (No arrows, but shouldn't send else where)
                    types.emplace_back(UIElement::Type::LOGINNOTICE);
                    // No tab (No arrows, but shouldn't send else where)
                    types.emplace_back(UIElement::Type::LOGINNOTICE_CONFIRM);
                    // No tab (No arrows, but shouldn't send else where)
            ...
            if (!sent) {
                if (escape) {
                    ...
                    if (auto chatbar = get().get_element<UIChatBar>())
    ...
    void UI::show_skill(Tooltip::Parent parent, int32_t skill_id, int32_t level, int32_t masterlevel,
                        int64_t expiration) {
    ...
    void UI::show_map(Tooltip::Parent parent, std::string title, std::string description, int32_t mapid, bool bolded,
                      bool portal) {
