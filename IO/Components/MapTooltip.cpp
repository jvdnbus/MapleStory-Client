...
namespace ms {
    ...
    MapTooltip::MapTooltip() : parent(NONE), title(""), description(""), fillwidth(0), fillheight(0) {
    ...
    void MapTooltip::set_title(Parent p, std::string t, bool bolded) {
        if (parent == p && title == t)
            return;
    ...
    void MapTooltip::set_worldmap_title(bool bolded) {
        fillwidth = 206;
        title_label = Text(bolded ? Text::Font::A12B : Text::Font::A12M, Text::Alignment::CENTER, Color::Name::WHITE,
                           title);
    ...
    void MapTooltip::set_minimap_mapid(std::unordered_map<int64_t, std::pair<std::string, std::string>> life,
                                       bool portal) {
        int32_t m = 0;
        int32_t n = 0;
        bool desc_empty = desc_label.empty();
