...
namespace ms {
    ...
    void EquipTooltip::set_equip(Parent parent, int16_t ivp) {
        ...
        if (invtype == InventoryType::Id::EQUIP) {
            ...
            if (inventory.has_equipped(eqslot));
            {
                ...
                if (eequip) {
                    ...
                    case 1:
                        okjobs_preview.push_back(1);
                        canequip_preview[MapleStat::Id::JOB] = (stats.get_stat(MapleStat::Id::JOB) / 100 == 1) || (stats
                            .get_stat(MapleStat::Id::JOB) / 100 >= 20);
                        break;
                    case 2:
                        okjobs_preview.push_back(2);
                    ...
                    for (EquipStat::Id es = EquipStat::Id::STR; es <= EquipStat::Id::JUMP; es = static_cast<
                             EquipStat::Id>(es + 1)) {
        ...
        case 1: // Warrior
            okjobs.push_back(1);
            canequip[MapleStat::Id::JOB] = (stats.get_stat(MapleStat::Id::JOB) / 100 == 1) || (stats.get_stat(
                MapleStat::Id::JOB) / 100 >= 20);
            break;
        case 2: // Magician
            okjobs.push_back(2);
        ...
        case 3: // Magician, Warrior
            okjobs.push_back(1);
            okjobs.push_back(2);
            canequip[MapleStat::Id::JOB] = (stats.get_stat(MapleStat::Id::JOB) / 100 == 1) || (stats.
                get_stat(MapleStat::Id::JOB) / 100 >= 20) || stats.get_stat(MapleStat::Id::JOB) / 100 == 2;
            break;
        case 4: // Bowman
            okjobs.push_back(3);
        ...
        for (EquipStat::Id es = EquipStat::Id::STR; es <= EquipStat::Id::JUMP; es = static_cast<EquipStat::Id>(es +
                 1)) {
            if (equip.get_stat(es) > 0) {
                int16_t delta = equip.get_stat(es) - equipdata.get_defstat(es);
                std::string statstr = std::to_string(equip.get_stat(es));
