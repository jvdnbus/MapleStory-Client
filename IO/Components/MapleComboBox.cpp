...
namespace ms {
    MapleComboBox::MapleComboBox(Type type, std::vector<std::string> o, uint16_t default_option, Point<int16_t> ppos,
                                 Point<int16_t> pos, int64_t w) : options(o), rwidth(w),
                                                                  selected_index(default_option), parentpos(ppos) {
        ...
        textures[PRESSED][0] = src["pressed"][0];
        textures[PRESSED][1] = src["pressed"][1];
        textures[PRESSED][2] = src["pressed"][2];

        textures[MOUSEOVER][0] = src["mouseOver"][0];
        textures[MOUSEOVER][1] = src["mouseOver"][1];
        textures[MOUSEOVER][2] = src["mouseOver"][2];

        textures[NORMAL][0] = src["normal"][0];
        textures[NORMAL][1] = src["normal"][1];
        textures[NORMAL][2] = src["normal"][2];

        textures[DISABLED][0] = src["disabled"][0];
        textures[DISABLED][1] = src["disabled"][1];
        textures[DISABLED][2] = src["disabled"][2];
    ...
    void MapleComboBox::draw(Point<int16_t>) const {
        if (active) {
            auto lpos = Point<int16_t>(position + parentpos);
            ...
            if (pressed) {
                Point<int16_t> pos = Point<int16_t>(position.x(),
                                                    position.y() + textures[state][0].get_dimensions().y()) + parentpos;
    ...
    Cursor::State MapleComboBox::send_cursor(bool clicked, Point<int16_t> cursorpos) {
        ...
        for (auto& btit : buttons) {
            if (btit.second->is_active() && btit.second->bounds(position).contains(cursorpos)) {
                if (btit.second->get_state() == NORMAL) {
                    Sound(Sound::Name::BUTTONOVER).play();

                    btit.second->set_state(MOUSEOVER);
                    ret = Cursor::State::CANCLICK;
                } else if (btit.second->get_state() == MOUSEOVER) {
                    if (clicked) {
