...
namespace ms {
    ...
    public:
        ...
        static constexpr GLfloat colors[NUM_COLORS][3] =
        {
        ...
        constexpr Color(float red, float green, float blue, float alpha) : Color(underlying_t{
            red, green, blue, alpha
        }) {
        }

        // Create a color by an array of natural numbers [0, 255]
        constexpr Color(const std::array<uint8_t, LENGTH> comps) : Color(comps[0], comps[1], comps[2], comps[3]) {
        }
