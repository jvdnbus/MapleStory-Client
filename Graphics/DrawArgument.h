...
namespace ms {
    ...
    public:
        ...
        constexpr DrawArgument(Point<int16_t> position, float xscale, float yscale) : DrawArgument(
            position, position, xscale, yscale, 1.0f) {
        }

        constexpr DrawArgument(Point<int16_t> position, Point<int16_t> stretch) : DrawArgument(
            position, position, stretch, 1.0f, 1.0f, 1.0f, 0.0f) {
        }

        constexpr DrawArgument(Point<int16_t> position, bool flip) : DrawArgument(position, flip, 1.0f) {
        }

        constexpr DrawArgument(float angle, Point<int16_t> position, float opacity) : DrawArgument(
            angle, position, false, opacity) {
        }

        constexpr DrawArgument(Point<int16_t> position, float opacity) : DrawArgument(position, false, opacity) {
        }

        constexpr DrawArgument(Point<int16_t> position, Color color) : DrawArgument(
            position, position, Point<int16_t>(0, 0), 1.0f, 1.0f, color, 0.0f) {
        }

        constexpr DrawArgument(Point<int16_t> position, bool flip, Point<int16_t> center) : DrawArgument(
            position, center, flip ? -1.0f : 1.0f, 1.0f, 1.0f) {
        }
        ...
        constexpr DrawArgument(float xscale, float yscale, float opacity) : DrawArgument(
            Point<int16_t>(0, 0), xscale, yscale, opacity) {
        }
        ...
        constexpr DrawArgument(Point<int16_t> position, bool flip, float opacity) : DrawArgument(
            position, position, flip ? -1.0f : 1.0f, 1.0f, opacity) {
        }

        constexpr DrawArgument(float angle, Point<int16_t> position, bool flip, float opacity) : DrawArgument(
            position, position, Point<int16_t>(0, 0), flip ? -1.0f : 1.0f, 1.0f, opacity, angle) {
        }
        ...
        constexpr DrawArgument(Point<int16_t> position, Point<int16_t> center, Point<int16_t> stretch, float xscale,
                               float yscale, Color color, float angle) : pos(position), center(center),
                                                                         stretch(stretch), xscale(xscale),
                                                                         yscale(yscale), angle(angle), color(color) {
        }
