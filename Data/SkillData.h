    ...
    public:
        ...
        struct Stats {
            ...
            constexpr Stats(float damage, int32_t matk, int32_t fixdamage, int32_t mastery, uint8_t attackcount,
                ...
                mobcount(mobcount), bulletcount(bulletcount), bulletcost(bulletcost), hpcost(hpcost), mpcost(mpcost),
                chance(chance), critical(critical), ignoredef(ignoredef), hrange(hrange), range(range) {
            }
        };
