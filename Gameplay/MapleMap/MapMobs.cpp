...
namespace ms {
    ...
    void MapMobs::apply_damage(int32_t oid, int32_t damage, bool toleft, const AttackUser& user,
    ...
    int32_t MapMobs::find_colliding(const MovingObject& moveobj) const {
        auto horizontal = Range<int16_t>(moveobj.get_last_x(), moveobj.get_x());
        auto vertical = Range<int16_t>(moveobj.get_last_y(), moveobj.get_y());
