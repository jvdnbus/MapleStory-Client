...
namespace ms {
    NpcSpawn::NpcSpawn(int32_t o, int32_t i, Point<int16_t> p, bool fl, uint16_t f) : oid(o), id(i), position(p),
                                                                                      flip(fl), fh(f) {
    }
    ...
    ReactorSpawn::ReactorSpawn(int32_t o, int32_t r, int8_t s,
                               Point<int16_t> p) : oid(o), rid(r), state(s), position(p) {
    }
    ...
    DropSpawn::DropSpawn(int32_t o, int32_t i, bool ms, int32_t ow, Point<int16_t> p, Point<int16_t> d, int8_t t,
                         int8_t m, bool pd) : oid(o), id(i), meso(ms), owner(ow), start(p), dest(d), droptype(t),
                                              mode(m), playerdrop(pd) {
    }
    ...
    CharSpawn::CharSpawn(int32_t cid, const LookEntry& look, uint16_t level, int16_t job, const std::string& name,
                         int8_t stance, Point<int16_t> position) : cid(cid), level(level), job(job), name(name),
                                                                   stance(stance), position(position), look(look) {
    }
